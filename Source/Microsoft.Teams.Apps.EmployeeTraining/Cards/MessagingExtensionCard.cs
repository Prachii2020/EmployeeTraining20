    ...
    public static class MessagingExtensionCard
    {
        ...
        public static MessagingExtensionResult GetCard(IEnumerable<EventEntity> events, string applicationBasePath, IStringLocalizer<Strings> localizer, DateTimeOffset? localDateTime)
        {
            ...
            foreach (var eventDetails in events)
            {
                ...
                var previewCard = GetThumbnailCard(eventDetails, localDateTime, localizer, applicationBasePath);
        ...
        /// <param name="applicationBasePath">Application base path url</param>
        /// <returns>Thumbnail card.</returns>
        private static ThumbnailCard GetThumbnailCard(EventEntity eventDetails, DateTimeOffset? localDateTime, IStringLocalizer<Strings> localizer, string applicationBasePath)
        {
            ...
            var text = (EventAudience)eventDetails.Audience == EventAudience.Private
                ? $"<span style='color: #A72037; font-weight: 600;'>{categoryString} &nbsp;|</span>" +
                $"<span style=' font-weight: 600;'>&nbsp;&nbsp;{locationString}</span>" +
                $"<img style='float: right;' alt='private event icon' width='32' height='32' src='{applicationBasePath}/images/PrivateIcon.png'/><br/>" +
                $"<span style='font-size: 11px; line-height: 22px;'>{trainingStartDateString}, {trainingStartTimeString}-{trainingEndTimeString}</span>"
                :
                $"<span style='color: #A72037; font-weight: 600;'>{categoryString} &nbsp;|</span>" +
                $"<span style=' font-weight: 600;'>&nbsp;&nbsp;{locationString}</span><br/>" +
                $"<span style='font-size: 11px; line-height: 22px;'>{trainingStartDateString}, {trainingStartTimeString}-{trainingEndTimeString}</span>";

            return new ThumbnailCard
            {
                Title = $"<span style='font-weight: 600;'>{titleString}</span>",
                Text = text,
            };
        }
    }
