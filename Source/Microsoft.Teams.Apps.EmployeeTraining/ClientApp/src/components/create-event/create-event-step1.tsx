...
import { uploadEventImage, getEventCategoriesAsync, searchEventAsync } from "../../api/create-event-api";
...
class CreateEventStep1 extends React.Component<ICreateEventsStep1Props, ICreateEventsStep1State> {
    ...
    UNSAFE_componentWillReceiveProps(nextProps: ICreateEventsStep1Props) {
        if (nextProps.eventPageState.eventDetails !== this.props.eventPageState.eventDetails) {
            this.setState({ eventDetails: { ...nextProps.eventPageState.eventDetails } }, this.getAllCategories);
        }
        ...
        if (nextProps.teamsContext && nextProps.teamsContext !== this.props.teamsContext) {
            ...
            if (!this.props.eventPageState.categories.length) {
    ...
    getAllCategories = async () => {
        ...
        if (response.status === ResponseStatus.OK) {
            ...
            if (this.props.eventPageState.eventDetails.categoryId) {
                let category = categories.find((category: any) => category.id === this.props.eventPageState.eventDetails.categoryId);
    ...
    */
    checkIfSaveAsDraftAllowed = () => {
        let eventValidationStatus = { isPhotoValid: true, isNameValid: true, isDescriptionValid: true, isEventTypeValid: true, isEventCategoryValid: true, isVenueValid: true, isMaxNoofPartipantsValid: true, isMeetingLinkValid: true };
        ...
        if (eventValidationStatus.isPhotoValid && eventValidationStatus.isNameValid &&
    ...
    */
    checkIfNextAllowed = async () => {
        let eventValidationStatus = { isTimeValid: true, isPhotoValid: true, isNameValid: true, isDescriptionValid: true, isEventTypeValid: true, isEventCategoryValid: true, isVenueValid: true, isMaxNoofPartipantsValid: true, isMeetingLinkValid: true };

        if (!this.state.eventDetails.photo) {
        ...
        if (this.state.eventDetails.description == "" || this.state.eventDetails.description.length > Resources.eventDescriptionMaxLength) {
    ...
    */
    uploadImage = async (formData: FormData) => {
        ...
        if (response && response.status === ResponseStatus.OK && response.data) {
            this.setState((prevState: ICreateEventsStep1State) => ({
                eventDetails: { ...prevState.eventDetails, photo: response.data },
                imageUploadLoader: false
            }));
        }
        else {
            ...
            <>
                <div className="page-content">
                    ...
                    <Flex gap="gap.smaller" className="margin-top">
                        <Text className="form-label margin-right" content={this.localize("eventPhotoStep1")} /><ExclamationCircleIcon title={this.localize("invalidImageDimensions")} size="small" />
                        <Flex.Item push>
                            {this.getPhotoError()}
                        </Flex.Item>
                    </Flex>
                    <Flex gap="gap.smaller" className="input-label-margin-between" vAlign="end">
                        {this.state.eventDetails.photo && <Flex.Item size="size.half" >
                            <FluentImage src={this.state.eventDetails.photo} className="upload-image" styles={{ maxWidth: "50vw" }} />
                        </Flex.Item>}
                        <Flex.Item>
                            <>
