...
class ManageEvents extends React.Component<IManageEventsProps, IManageEventsState> {
     ...
     */
    componentWillReceiveProps(nextProps: IManageEventsProps) {
        if (this.props.teamsContext !== nextProps.teamsContext) {
            if (nextProps.teamsContext) {
                this.teamId = nextProps.teamsContext.teamId!;

                let activeEvents = this.getEventsAsync(0, EventSearchType.ActiveEventsForTeam);
                let draftEvents = this.getEventsAsync(0, EventSearchType.DraftEventsForTeam);
                let completedEvents = this.getEventsAsync(0, EventSearchType.CompletedEventsForTeam);

                Promise.all([activeEvents, draftEvents, completedEvents])
                    .then((results) => {
                        this.setTotalEventsCount(EventSearchType.ActiveEventsForTeam, results[0].length);
                        this.setTotalEventsCount(EventSearchType.DraftEventsForTeam, results[1].length);
                        this.setTotalEventsCount(EventSearchType.CompletedEventsForTeam, results[2].length);
                    })
                    .finally(() => {
                        this.getEvents();
                    });
