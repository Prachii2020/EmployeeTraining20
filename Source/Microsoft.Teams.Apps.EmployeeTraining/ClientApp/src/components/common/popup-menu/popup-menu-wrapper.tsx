    ...
    if (props.checkboxes) {
        return (
            <Popup
                ...
                trigger={<Button data-testid={props.checkboxes[0].title + "_CheckboxButton"} className="filter-button" onClick={() => onFilterClick()} content={<Text content={props.title} />} iconPosition="after" icon={<ChevronDownIcon />} text />}
        ...
        return (
            <Popup
                ...
                position="below"
                onOpenChange={(e, { open }: any) => onPopupOpenChange(open)}
                trigger={<Button data-testid={props.radioGroup[0].name + "_RadioGroupButton"} icon={<ChevronDownIcon />} className="filter-button" iconPosition="after" content={<Text content={props.title} />} text />}
