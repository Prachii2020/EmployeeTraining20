...
namespace Microsoft.Teams.Apps.EmployeeTraining.Helpers
{
    ...
    public class EventWorkflowHelper : IEventWorkflowHelper
    {
        ...
        /// </summary>
        private IEventGraphHelper eventGraphHelper;
        ...
        public async Task<bool?> UpdateDraftEventAsync(EventEntity eventEntity)
        {
            ...
            var result = await this.eventRepository.UpsertEventAsync(eventToUpdate);
            if (result)
            {
                await this.eventSearchService.RunIndexerOnDemandAsync();
            }
        ...
        public async Task<bool?> CreateNewEventAsync(EventEntity eventEntity, string createdByName)
        {
            ...
            if (!string.IsNullOrEmpty(eventEntity.EventId))
            {
                ...
                this.MapEventModel(eventEntity, eventToUpsert, false, false);
            }
            else
            {
                // Generate new event Id.
                eventToUpsert.EventId = Guid.NewGuid().ToString();
            }

            // Calculate occurrences as per start and end date.
            eventToUpsert.NumberOfOccurrences = this.GetWeekDaysCount(eventEntity.StartDate.Value, eventEntity.EndDate.Value);
            ...
            await this.SendEventCreationNotificationInTeam(eventToUpsert);
            if (result)
            {
        ...
        public async Task<bool?> UpdateEventAsync(EventEntity eventEntity)
        {
            ...
            var existingAutoRegisteredAttendees = string.IsNullOrEmpty(eventToUpsert.AutoRegisteredAttendees)
                ? new List<string>() : eventToUpsert.AutoRegisteredAttendees.Split(";").ToList();
        ...
        public async Task<bool?> DeleteDraftEventAsync(string teamId, string eventId)
        {
            ...
            bool result = await this.eventRepository.UpsertEventAsync(eventToUpdate);
        ...
        public async Task<bool> CloseEventRegistrationsAsync(string teamId, string eventId, string userAadId)
        {
            ...
            bool result = await this.eventRepository.UpsertEventAsync(eventDetails);
        ...
        public async Task<List<string>> SendReminderAsync(string teamId, string eventId)
        {
            var eventDetails = await this.eventRepository.GetEventDetailsAsync(eventId, teamId);
            ...
            var notificationFailedUserdIds = this.notificationHelper.BroadcastNotificationToUsers(registeredAttendees, notificationCard);

            return notificationFailedUserdIds;
        }
        ...
        private async Task<bool> SendCancellationNotificationAsync(string teamId, string eventId)
        {
            ...
            var notificationCard = EventCancellationCard.GetCancellationCard(this.botOptions.Value.AppBaseUri, this.localizer, eventDetails, this.botOptions.Value.ManifestId);
            this.notificationHelper.BroadcastNotificationToUsers(registeredAttendees, notificationCard);
        ...
        private void MapEventModel(EventEntity sourceEventEntity, EventEntity destinationEventEntity, bool isEdit, bool isDraft)
        {
            ...
            destinationEventEntity.Name = sourceEventEntity.Name;
            ...
            if (!isEdit)
            {
                ...
                destinationEventEntity.EventId = Guid.NewGuid().ToString();
        ...
        /// <param name="eventDetails">Event details.</param>
        /// <returns>Returns true if notification sent successfully. Else returns false.</returns>
        private async Task<bool> SendEventCreationNotificationInTeam(EventEntity eventDetails)
        {
            if (eventDetails == null)
            {
                return false;
            ...
            await this.categoryHelper.BindCategoryNameAsync(new List<EventEntity>() { eventDetails });
            await this.teamConfigurationRepository.GetTeamDetailsAsync(eventDetails.TeamId);

            // TODO: send event creation card in team
            return true;
        }
