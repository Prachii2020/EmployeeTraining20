                    ...
                    try
                    {
                        await this.retryPolicy.ExecuteAsync(async () =>
                        {
                            ...
                            await ((BotFrameworkAdapter)this.botAdapter).ContinueConversationAsync(
                                ...
                                async (context, token) =>
                                {
                                    var teamMembers = await TeamsInfo.GetTeamMembersAsync(context, teamDetails.TeamId, CancellationToken.None);

                                    if (teamMembers != null && teamMembers.Any())
                                    {
                                        allLnDTeamMembers.AddRange(teamMembers);
                                    }
                                }, default);
                        });
                    }
...
#pragma warning restore CA1031 // Catching general exceptions to log exception details in telemetry client.
            ...
            this.cache.Set("all-team-members", allLnDTeamMembers, TimeSpan.FromMinutes(30));
