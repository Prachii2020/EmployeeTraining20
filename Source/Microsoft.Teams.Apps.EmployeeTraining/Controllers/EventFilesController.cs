...
#pragma warning restore CA1801 // Required to validate whether user is part of particular team.
        {
            ...
            if (fileInfo == null)
            {
                ...
                return this.BadRequest(new { message = "File information received for uploading to blob is null." });
            }
        ...
        public async Task<ActionResult> ExportEventDetailsToCSV(string teamId, string eventId)
        {
            if (string.IsNullOrEmpty(teamId))
            {
                ...
                this.RecordEvent("Export Event Details- The HTTP GET call to export event details to CSV has been failed");
            ...
            try
            {
                this.RecordEvent("Export Event Details- The HTTP GET call to export event details to CSV has been initiated");

                var csvData = await this.eventWorkflowHelper.ExportEventDetailsToCSVAsync(teamId, eventId);

                this.RecordEvent("Export Event Details- The HTTP GET call to export event details to CSV has succeeded");
            ...
            catch (Exception ex)
            {
                ...
                this.RecordEvent($"Export Event Details- The HTTP GET call to export details for event {eventId} has failed");
                throw;
            }
        }
    }
