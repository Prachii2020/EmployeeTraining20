...
namespace Microsoft.Teams.Apps.EmployeeTraining.Controllers
{
    ...
    public class EventController : BaseController
    {
        ...
        public async Task<IActionResult> GetEventAsync(string eventId, string teamId)
        {
            ...
            this.RecordEvent("Get event- The HTTP POST call to get event details has been initiated");
            try
            {
                ...
                this.RecordEvent("Get event- The HTTP POST call to get event details has been succeeded");
                return this.Ok(eventDetails);
            }
            catch (Exception ex)
            {
                this.RecordEvent("Get event- The HTTP POST call to get event details has been failed");
        ...
        public async Task<IActionResult> GetEventsAsync(string searchString, int pageCount, int eventSearchType, string createdByFilter, string categoryFilter, int sortBy)
        {
            if (!Enum.IsDefined(typeof(EventSearchType), eventSearchType))
            {
                ...
                return this.BadRequest("The event search type was invalid");
            }

            if (!Enum.IsDefined(typeof(SortBy), sortBy))
            {
                this.logger.LogError("Invalid sort by value");
                return this.BadRequest("Provided sort by value was invalid");
            }
        ...
        public async Task<IActionResult> SearchEventAsync(string search)
        {
            if (string.IsNullOrEmpty(search))
            {
                ...
                return this.BadRequest(new { message = "Search query is either null or empty" });
            }
        ...
        public async Task<IActionResult> RegisterToEventAsync(string teamId, string eventId)
        {
            ...
            if (string.IsNullOrEmpty(eventId))
            {
                ...
                return this.BadRequest("Invalid event Id was provided");
            }

            try
            {
                this.RecordEvent("Register to event- The HTTP POST call to register user for an event has initiated");

                var isRegistrationSuccessful = await this.userEventsHelper.RegisterToEventAsync(teamId, eventId, this.UserAadId);

                this.RecordEvent("Register to event- The HTTP POST call to register user for an event has succeeded");
            ...
            catch (Exception ex)
            {
                ...
                this.RecordEvent($"Register to event- The HTTP POST call to register user {this.UserAadId} for event {eventId} was failed");
                throw;
            }
        }
        ...
        public async Task<IActionResult> UnregisterToEventAsync(string teamId, string eventId)
        {
            ...
            try
            {
                this.RecordEvent("Unregister to event- The HTTP POST call to unregister user to an event has initiated");

                var isUserRemovedFromEvent = await this.userEventsHelper.RemoveEventAsync(teamId, eventId, this.UserAadId);

                this.RecordEvent("Unregister to event- The HTTP POST call to unregister user to an event has succeeded");
            ...
            catch (Exception ex)
            {
                ...
                this.RecordEvent($"Unregister to event- The HTTP POST call to unregister user {this.UserAadId} to event {eventId} was failed");
                throw;
            }
        }
    }
